...
of ``No One Size Fits All'' in designing databases~\citeref{stonebraker2005one}, recent
...
ways. For example, imMens~\citeorgref{Liu:2013:IRT}, profiler~\citeorgref{Kandel:2012:PIS}, and nanocubes~\citeorgref{Lins:2013:NFR} have
shown that careful algorithm design can enable extremely fast
interactive visualizations of massive data. Scalr~\citeorgref{Battle:2013:DRO} has shown that
traditional query planners can fruitfully take advantage of perceptual
and resolution limits. ForeCache~\citeorgref{Battle:2015:VLDB} utilizes a predictive prefetching
...
SampleAction~\citeorgref{Fisher:2012:TMI} takes advantage of streaming databases and
...
operations across columns (instead of the traditional rows). BlinkDB~\citeref{Agarwal:2013:BQW}
uses a sampling strategy to build a smaller, but representative
database from a larger one. Map-D~\citeref{mapd} takes advantage of modern
graphics card capabilities and resulted in a specialized database that
runs entirely within (an array of) graphics cards. Finally, SciDB~\citeref{cudre2009demonstration}
...
years at VIS, most notably by the BELIV workshop~\citeref{beliv}.
